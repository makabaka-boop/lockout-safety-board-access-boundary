...
- `test/api.test.ts`：角色鉴权、建票校验（1–20 点、授权人员）、确认、挂 / 撤锁、
  乐观修订号 409、复位阻断项与终态；
- `test/security.test.ts`：授权边界与越权旁路泄露——检修人员列表只见被授权的票、
  未授权详情/人员目录/三个写入口一律 403 且错误体只含 `code`+`message`
  （无快照/修订号/阻断项）、被拒写请求数据库零变更、全部 `/api` 响应
  `Cache-Control: no-store`，并回归合法角色与合法并发冲突快照；
- `test/concurrency.test.ts`：两实例"撤最后一锁 × 复位"同修订号并发 10 轮、
  4 个复位并发仅 1 次成功、20 张票交错重放每张票恰好送电一次；并断言败方所见
  快照（revision / status / 锁数 / 阻断项）与其裁决时刻的数据库真值一致；
- `test/ui/ui.test.tsx`：jsdom + Testing Library 渲染真实 React 页面，所有
  `/api` 请求打到真实 Fastify + PG；覆盖旧页面收到 409 后界面自动重载到并发者
  的结果、刷新后阻断项消失、复位终态，以及未授权账号下列表/直达牌板都不渲染、
  不短暂暴露任何快照字段。
...
**快照只随 409 并发冲突下发给已通过票级授权的调用者**（供其页面立即重载）；
未授权的 403（含越权写请求）与 404 错误体仅含 `code` 与 `message`，不暴露
票是否存在之外的任何业务数据（无 `snapshot` / `latestRevision` / `blockers`）。
所有 `/api/*` 响应（成功与失败）均带 `Cache-Control: no-store`。

## 授权模型（读、写、人员目录）

- 协调员：可见全部票、建票、调用人员目录（`/api/workers`）；
- 送电负责人：可见全部票（裁决需要）、在满足条件时复位送电；
- 检修人员：**只能列出与读取授权名单包含自己的票**，仅在被授权的票上确认点、
  挂 / 撤自己的个人锁；人员目录、他人票的详情、任何票号上的越权写请求一律
  返回无业务数据的 403，且拒绝不改写修订号或牌板。
